...
setwd("~/Library/Mobile\ Documents/com~apple~CloudDocs/Violence Project")
load("all_deaths_16.RData")

homicides_16 <- deaths_16 %>%
  filter(cause_recode358 %in% c("432","433","434","435","436","437","438","439","440","441"))
...
masskills <- data %>%
  filter(VicCount > 3 & ActionType == 'Normal update' & Weapon %in% c("Fire", "Explosives", "Firearm, type not stated",
                                                                      "Firearm, type not stated","Other gun","Rifle","Shotgun"))

masskills$Weapon[c("Fire","Explosives"]

weaponYear <- aggregate(VicCount ~ Weapon + Year, data=masskills, sum)

aggregate(VicCount ~ Weapon, data=masskills_2000s, sum)

aggregate(VicCount ~ Year, data=masskills, sum)

ggplot(data = masskills, aes(Year)) +
  geom_bar() +
  facet_grid(~Weapon)


ggplot(data = homicides_16, aes(sex)) +
  geom_hex() +
  facet_grid(~race)
